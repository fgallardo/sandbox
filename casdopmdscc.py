...
def main():
	...
	plt.plot(freq*((c0+(dist(dx,dy,dz)*1000))/c0)/1e9) 
	plt.title("Doppler effect due to relative speed between Cassini and DSS-63")
	plt.ylabel("GHz")
	plt.xlabel(" ")#"Samples")
	plt.grid()
	fig=plt.figure()
	ax=fig.add_subplot(111,projection='3d')
	ax.plot(x,y,z,label="Cassini to MDSCC")
	plt.legend()
	plt.show()
